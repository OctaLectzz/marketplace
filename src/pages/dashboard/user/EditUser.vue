    ...
    <q-form @submit="createData">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h6">Edit Pengguna</div>
          <q-space />
...
const rules = ref({
  username: [
    ...
    (v) => {
      if (typeof v === 'string') {
        if (users.value) {
          return !users.value.some((user) => user.username.toLowerCase() === v.toLowerCase() && user.username.toLowerCase() !== item.username.toLowerCase()) || 'Username sudah ada'
        }
      }
      return true
    }
  ...
  email: [
    ...
    (v) => {
      if (typeof v === 'string') {
        if (users.value) {
          return !users.value.some((user) => user.email.toLowerCase() === v.toLowerCase() && user.email.toLowerCase() !== item.email.toLowerCase()) || 'Email sudah ada'
        }
      }
      return true
    }
...
const createData = async () => {
  ...
  try {
    ...
    await useUserStore().edit(data.value)
    ...
    $q.notify({
      message: error.response.data.message || 'Pengguna gagal diedit, Silahkan coba lagi nanti',
      icon: 'warning',
      color: 'negative'
    })
  }
